...
def learn_and_save(model, name, eval_env, **kwargs):
    ...
    eval_callback = EvalCallback(eval_env, eval_freq=1000, n_eval_episodes=5, log_path=None, best_model_save_path=None)
...
# Using the hyperparameters from the original papers
algorithms = {
    ...
    "ppo": partial(SAC, "MlpPolicy", learning_rate=0.0003, buffer_size=1000000, learning_starts=100, batch_size=256, tau=0.005, gamma=0.99, train_freq=1, gradient_steps=1, verbose=0, tensorboard_log = tensorboard_logs),
...
continuous_tests = {
    f"{args.algo}_{env}_{time_steps}_{args.seed}": (algorithms[args.algo](env=gym.make(env), seed = args.seed), env)
    for env in 
    envs
}
...
with ThreadPoolExecutor() as executor:
    futures = [executor.submit(learn_and_save, model, name, gym.make(env), total_timesteps=time_steps) for name, (model, env) in continuous_tests.items()]
